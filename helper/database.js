...
class Database {
    ...
    viewAllEmployees() {
        return this.connection.query(
            "SELECT = FROM employees JOIN roles ON roles.id = employees.role_id JOIN department ON roles.department_id = department.id JOIN employees e ON e.id = employees.manager_id");
}
